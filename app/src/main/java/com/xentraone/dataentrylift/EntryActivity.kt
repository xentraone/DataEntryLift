...
class EntryActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Safe area: toolbar absorbs the status bar, the root absorbs the
        // navigation bar and the keyboard so focused fields stay visible.
        val root = findViewById<View>(R.id.rootEntry)
        val toolbarPadTop = toolbar.paddingTop
        ViewCompat.setOnApplyWindowInsetsListener(root) { _, insets ->
            val bars = insets.getInsets(WindowInsetsCompat.Type.systemBars())
            val ime = insets.getInsets(WindowInsetsCompat.Type.ime())
            toolbar.updatePadding(top = toolbarPadTop + bars.top)
            root.updatePadding(bottom = maxOf(bars.bottom, ime.bottom))
            insets
        }
